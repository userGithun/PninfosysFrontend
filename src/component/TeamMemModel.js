...
export default function TeamMemModel({
    isOpen,
    closeModal,
    formData,
    handleChange,
            ...
            <div className="bg-white w-full max-w-md rounded shadow-lg p-6 relative">
                <div className="flex justify-between items-center mb-4 border-b pb-2">
                    <h2 className="text-lg font-semibold text-gray-800">
                        {formData._id ? 'Edit Member' : 'Add New Member'}
                    </h2>
                    <button
                        onClick={closeModal}
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-gray-700 mb-1">Desigation (Role)</label>
                        <input
                            type="text"
                            name="desigation"
                            value={formData.desigation}
                            onChange={handleChange}
                            required
                            className="w-full px-4 py-2 border border-gray-300 rounded focus:outline-none focus:ring focus:ring-blue-500"
                        />
